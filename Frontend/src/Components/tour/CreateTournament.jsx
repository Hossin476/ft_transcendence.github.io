...
const creatTournament = async (tokens, tournamentName, setTours,setTournamentName,status,setError,navigate)=> {
    if(status === "") {
        setError(()=>"mode")
        return
    }
    ...
    return (
        <div className=" h-[10rem] w-[100%] flex items-center justify-center">
            <div className="w-[90%] h-[5rem] flex bg-secondaryColor items-center relative justify-between border-[2px] border-thirdColor rounded-md px-4">
                ...
                <button onClick={()=>creatTournament(tokens, tournamentName, setTours,setTournamentName,status,setError,navigate)} className="xsm:text-xs xsm:ml-[10px] xsm:p-[2px] md:p-2  bg-secondaryColor  border-[1px] border-thirdColor rounded-md font-bold rounded ">Create Tournament</button>
                {
